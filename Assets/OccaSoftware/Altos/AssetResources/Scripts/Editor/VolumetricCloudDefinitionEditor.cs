...
namespace OccaSoftware.Altos.Editor
{
    ...
    public class VolumetricCloudDefinitionEditor : UnityEditor.Editor
    {
		...
		void DrawCloudDefinitionHeader()
        {
			...
			if (GUILayout.Button("Common", Params.Editor.pageSelection.enumValueIndex == 0 ? bold : style))
            {
                Params.Editor.pageSelection.enumValueIndex = 0;
            }
			if (GUILayout.Button("Low Altitude", Params.Editor.pageSelection.enumValueIndex == 1 ? bold : style))
            {
                Params.Editor.pageSelection.enumValueIndex = 1;
            }

            if (GUILayout.Button("High Altitude", Params.Editor.pageSelection.enumValueIndex == 2 ? bold : style))
            {
